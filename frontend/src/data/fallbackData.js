// Mirrors backend/database/seeders/PortfolioSeeder.php exactly.
// Used as a graceful fallback when the Laravel API isn't reachable
// (e.g. running the frontend standalone during development).
...
export const skills = {
  Languages: ['PHP', 'JavaScript', 'Python', 'HTML5', 'CSS3', 'SQL'].map((name) => ({ name })),
  Backend: ['Laravel', 'Laravel Sanctum', 'REST API Design', 'MVC Architecture', 'Service Layer Architecture', 'Authentication & Authorization', 'CodeIgniter', 'Node.js'].map((name) => ({ name })),
  Frontend: ['React.js', 'jQuery', 'AJAX', 'Tailwind CSS'].map((name) => ({ name })),
  Database: ['MySQL', 'PostgreSQL', 'ClickHouse', 'Database Design & Optimization'].map((name) => ({ name })),
  Tools: ['Git', 'GitHub', 'SVN', 'Postman'].map((name) => ({ name })),
  'Cloud & DevOps': ['Docker', 'Kubernetes'].map((name) => ({ name })),
};

export const experiences = [
  {
    id: 1,
    company_name: 'Biz Secure Labs (Net Protector Antivirus)',
    role: 'Software Engineer (Full Stack Developer)',
    start_date: '2024-06-01',
    end_date: null,
    location: 'Pune, Maharashtra, India',
    description:
      'Full stack software engineer building enterprise security web portals and REST APIs consumed by MFC desktop clients, in a security-product environment with structured senior code review.',
    responsibilities: [
    ...
    impact_metrics: ['20+ application modules shipped', '15+ production REST APIs', '2 datastores integrated (MySQL, ClickHouse)'],
...
export const projects = [
  {
    ...
    tech_stack: ['Laravel', 'PHP', 'MySQL', 'REST APIs', 'Laravel Sanctum', 'jQuery', 'JavaScript', 'AJAX', 'MVC', 'Service Layer'],
    challenges: [
    ...
    outcome_metrics: ['Centralized rule management across remote scanning systems', 'Full edit history for every rule', 'Primary developer for backend + frontend integration'],
    screenshots: [],
  ...
  },
  {
    ...
    outcome_metrics: ['Resolved large-file upload failures', 'Structured metadata for every stored sample', 'Improved backend performance for bulk handling workflows'],
    screenshots: [],
    github_url: 'https://github.com/K871git',
    live_url: null,
  },
  {
    ...
    challenges: ['Building authentication from scratch without Sanctum or a framework auth package.'],
    trade_offs: ["Hand-rolled session auth instead of a package — slower to build, but forced a real understanding of what those packages abstract away."],
    lessons_learned: [
    ...
    outcome_metrics: ['Full custom auth implementation', 'Cross-framework fundamentals validated (CodeIgniter + PostgreSQL)'],
    screenshots: [],
    github_url: 'https://github.com/K871git',
    live_url: null,
  },
